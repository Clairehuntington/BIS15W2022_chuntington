...
```{r}
elephants %>% 
  ggplot(aes(x=sex, y=age)) + geom_point(na.rm=T)
```

3. What is the relationship between age and height?
```{r}
elephants %>% 
  ggplot(aes(x=height, y=age)) + geom_point(na.rm=T) + geom_smooth(method="lm")
```
...
```{r}
homerange %>% 
  ggplot(aes(x=log10.hra, y=log10.mass), size=log10.mass) + geom_point(na.rm=T)
