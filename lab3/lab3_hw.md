...
```r
sleep <- data.frame(msleep, rows=FALSE)
```

3. What are the dimensions of this data frame (variables and observations)? How do you know? Please show the *code* that you used to determine this below. 
There are 83 rows and 11 collums as I found with the code below.
...
I think there are NAs in some of the data as I determined below using the code (is.na.data.frame) for msleep. Where it says TRUE, I think we can assume that those are the places with NA in the data. 

```r
(is.na.data.frame(msleep))
```
...
There seem to be 32 herbivores represented in the data.
...
```r
large_mammals <-subset(msleep,bodywt >= 200)
```


```r
small_mammals <- subset(msleep, bodywt <=1)
```
...
```r
sleepiest_mammal <- sleep$name[max_sleep]
sleepiest_mammal
```
